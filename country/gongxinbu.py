...
def get_ctitle(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http', file_href):
            ...
            newfile_href = 'http://www.miit.gov.cn/' + file_href.replace('../', '')
    ...
    #         img_href = 'http://www.miit.gov.cn/' + img_href.replace('../','')
    #     else:
    #         href_add = href.replace(href.split('/')[-1], '')
...
#国家工信部文件公示    静态网页
def wjgs_url(row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    source1 = '工信部'
    source2 = '文件公示'
    source3 = '通知公告'
    ...
    top = soup.find_all('td',valign='top')[1]
    li_list = top.find_all('li')
    for li in li_list:
        li = str(li).replace('\n', '').replace('\r', '')
        ...
        if complete_href in href_bloom:
            ...
            #获取完整标题，附件（在分页面获取的）
            ctitle,file_names,img_names,css_names = get_ctitle(html_str,complete_href,file_ad)
            #插入数据库
...
def wjfb_url(row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    for tr in trs:
        try:
            ...
            title = ele_a.text
            #获取发布日期
            date = tr.find_element_by_xpath('td[5]').text.strip()
            date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
            date = datetime.datetime.strptime(date, '%Y-%m-%d')
            chref_list.append(complete_href)
            if complete_href in href_bloom:
...
#国家工信部政策解读   静态加载
def zcjd_url(row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    source1 = '工信部'
    source2 = '政策解读'
    source3 = '政策解读'
    print("网站：", source1 + '  ' + source2 + ' ' + url)
...
def main(row,worksheet,href_bloom,file_ad1,ProgramStarttime):
    href_list = []
    url1 = 'http://www.miit.gov.cn/n1146295/n7281310/index.html'
    row, chref_list = wjgs_url(row, worksheet, url1, href_bloom, file_ad1, ProgramStarttime)
    # try:
    #     row, chref_list = wjgs_url(row, worksheet, url1,href_bloom,file_ad1,ProgramStarttime)
    # except:
    #     try:
    ...
    # print(row)
    url2 = 'http://www.miit.gov.cn/gdnps/wjfbindex.jsp'
    try:
        ...
        try:
            ...
            print("报错信息：", e)
    href_list.extend(chref_list)
    url3 = 'http://www.miit.gov.cn/n1146295/n7281315/index.html'
    try:
        row, chref_list = zcjd_url(row,worksheet,url3,href_bloom,file_ad1,ProgramStarttime)
    except:
        try:
            time.sleep(15)
            row, chref_list = zcjd_url(row, worksheet, url3,href_bloom,file_ad1,ProgramStarttime)
