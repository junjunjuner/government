...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<!--头部s-->.*?<!--头部e-->','',html_str,flags=re.S)
    html_str = re.sub('<!--尾部s-->.*?<!--尾部e-->','',html_str,flags=re.S)
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.gddrc.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.gddrc.gov.cn' + img_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.gddrc.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
#广东省发展和改革委员会 通知公告+政策文件+政策解读    静态网页
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    ul = soup.find('ul', {'class': 'comlist1 mt10 '})
    #日期，链接，标题，标题
    href_list = ul.find_all('li',{'class': None})
    for h in href_list:
        date = h.span.text[2:-1]
        date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
        date = datetime.datetime.strptime(date, '%Y-%m-%d')
        href_re = h.a['href']
        if '../../' in href_re:
            complete_href = 'http://www.gddrc.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
        ...
        down_href = 'http://www.gddrc.gov.cn'
        title = h.a.text
...
def main(row, worksheet, href_bloom, file_ad1, ProgramStarttime):
    href_list = []
    url_list = ['http://drc.gd.gov.cn/ywgg/index.html','http://drc.gd.gov.cn/gfxwj5633/index.html',
                'http://drc.gd.gov.cn/zcjd5635/index.html']
    source1 = '省发改委'
    source2_list = ['公告公示','政策文件','政策解读']
