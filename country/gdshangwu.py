...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<!--头部开始-->.*?<!--头部结束-->','',html_str,flags=re.S)
    html_str = re.sub('<!--尾部开始-->.*?<!--尾部结束-->','',html_str,flags=re.S)
    html_str = re.sub('<!-- baidu BEGIN -->.*?<!-- baidu Button END -->','',html_str,flags=re.S)
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.gdcom.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.gdcom.gov.cn/' + img_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    #获取css文件信息,并下载
    css_infos = bsObj.find_all("link", {"rel":"stylesheet","href": re.compile(r'.css$')})
    css_names = []
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.gdcom.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    ul = soup.find('ul', {'class': 'comlist3'})
    #日期，链接，标题，标题
    href_list = re.findall(
        '<a href="(.*?)" title="(.*?)">(.*?)</a><span>(.*?) </span>', str(ul))
    for i in range(len(href_list)):
        date = href_list[i][-1]
        date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
        date = datetime.datetime.strptime(date, '%Y-%m-%d')
        href_re = href_list[i][0]
        if '../../' in href_re:
            complete_href = 'http://www.gdcom.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
        ...
        down_href = 'http://www.gdcom.gov.cn'
        title = href_list[i][-2]
