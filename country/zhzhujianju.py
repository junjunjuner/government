...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<div id="top">.*?<!-- header end -->','',html_str,flags=re.S)
    html_str = re.sub('<!-- baidu button begin -->.*?<!-- container end -->','',html_str,flags=re.S)
    html_str = re.sub('<!-- 代码 开始 -->.*?<!-- 代码 结束 -->', '', html_str, flags=re.S)
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.zhzgj.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.zhzgj.gov.cn/' + img_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.zhzgj.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    for h in href_list:
        ...
        href_re = h.a['href']
        if '../../' in href_re:
            complete_href = 'http://www.zhzgj.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
        ...
        down_href = 'http://www.zhzgj.gov.cn'
        title = h.a.text
...
def main(row, worksheet, href_bloom, file_ad1, ProgramStarttime):
    href_list = []
    url_list = ['http://www.zhzgj.gov.cn/xxgk/tzgg/']
    source1 = '住建局'
    source2_list = ['通知公告']
    source3_list = ['通知公告']
