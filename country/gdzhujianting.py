...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<div class="hearder wrapper ">(.*?)<script>','',html_str,flags=re.S)
    html_str = re.sub('<div class="footer mt30">(.*?)<script type="text/javascript">','',html_str,flags=re.S)
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.gdcic.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.gdcic.gov.cn' + img_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.gdcic.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    time.sleep(5)
    html_str = driver.page_source
    soup = beautiful(html_str, 'lxml')
    ul = soup.find('ul', {'class': 'news-group-list onlyTitle'})
    #日期，链接，标题，标题
    href_list = ul.find_all('li',{'class': None})
    for h in href_list:
        date = h.find_all('div',{'class':'news-group-date'})
        date = re.findall('>(.*?) ',str(date[0]))[0]
        date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
        date = datetime.datetime.strptime(date, '%Y-%m-%d')
        href_re = h.a['href']
        complete_href ='http://www.gdcic.gov.cn'+ href_re
        down_href = 'http://www.gdcic.gov.cn'
        title = h.a.text
...
def main(row, worksheet, href_bloom, file_ad1, ProgramStarttime):
    href_list = []
    url_list = ['http://www.gdcic.gov.cn/ZWGK/column_GSGG','http://www.gdcic.gov.cn/ZWGK/column_WJTZ']
    source1 = '住建厅'
    source2_list = ['公示公告','文件通知']
    source3_list = ['通知公告','通知公告']
