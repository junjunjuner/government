...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<table width="1002" border="0" align="center" cellpadding="0" cellspacing="0".*?</table>','',html_str,flags=re.S)
    html_str = re.sub('<table width="924" border="0" align="center" cellpadding="0" cellspacing="0">.*?</table>','',html_str,flags=re.S)
    html_str = re.sub('<td width="39" align="left" valign="top">.*?</td>','',html_str,flags=re.S)
    # html_str = re.sub('<img.*? />', '',html_str, flags=re.S)
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.zhdpb.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.zhdpb.gov.cn/' + img_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.zhdpb.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    ul = soup.find('div', {'class': 'list'})
    #日期，链接，标题，标题
    href_list = ul.find_all('li')
    for h in href_list:
        ...
        href_re = h.a['href']
        if '../../' in href_re:
            complete_href = 'http://www.zhdpb.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
        ...
        down_href = 'http://www.zhdpb.gov.cn'
        title = h.a.text
...
def main(row, worksheet, href_bloom, file_ad1, ProgramStarttime):
    href_list = []
    url_list = ['http://www.zhdpb.gov.cn/zwgk/tzgg/','http://www.zhdpb.gov.cn/zwgk/fzgh/',
                'http://www.zhdpb.gov.cn/zwgk/zcwj/']
    source1 = '发改局'
    source2_list = ['通知公告','发展规划','政策文件']
    source3_list = ['通知公告','工作动态','政策法规']
