...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.gdei.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.gdei.gov.cn' + img_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.gdei.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
#广东省经济和信息化委员会 通知公告+项目资金+政策规划    静态网页
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    ul = soup.find('ul', {'class': 'NewsList'})
    #日期，链接，标题，标题
    href_list = re.findall(
        '<span>(.*?)</span><a href="(.*?)" target="_blank" title="(.*?)">(.*?)</a>', str(ul))
    for i in range(len(href_list)):
        date = href_list[i][0]
        date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
        date = datetime.datetime.strptime(date, '%Y-%m-%d')
        href_re = href_list[i][1]
        if '../../' in href_re:
            complete_href = 'http://www.gdei.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
        ...
        down_href = 'http://www.gdei.gov.cn'
        title = href_list[i][-1]
...
def main(row,worksheet,href_bloom,file_ad1,ProgramStarttime):
    href_list = []
    url_list = ['http://www.gdei.gov.cn/zwgk/tzgg/index.htm','http://www.gdei.gov.cn/zwgk/xmzj/index.htm',
                'http://www.gdei.gov.cn/ywfl/zcgh/index.htm']
    source1 = '经信委'
    source2_list = ['通知公告','项目资金','政策规划']
    source3_list = ['通知公告','通知公告','政策法规']
    for i in range(1):
        url = url_list[i]
