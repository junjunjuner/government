...
def getHtml_quiet(url):
    ...
    try:
        ...
        chard = 'utf8'
        html_str = html.decode(chard, 'ignore')
    html_str = re.sub('<!-- 通用头部开始 -->.*?<!-- 通用头部结束 -->','',html_str,flags=re.S)
    html_str = re.sub('<!-- 通用尾部开始 -->.*?<!-- 通用尾部结束 -->','',html_str,flags=re.S)
    html_str = re.sub('<td width="36" align="left" valign="top">.*?</td>','',html_str,flags=re.S)
    html_str = re.sub('<table width="960" height="156" border="0" align="center" cellpadding="0" cellspacing="0">.*?</table>','',html_str,flags=re.S)
    html_str = re.sub('<table width="960" height="22" border="0" align="center" cellpadding="0" cellspacing="0">.*?</table>','',html_str,flags=re.S)
    html_str = re.sub('<td align="left" valign="top"><img src=".*?images/2010-ZHKGMXJ-images-67_r1_c1.jpg" width="960" height="9" /></td>','',html_str)
    html_str = re.sub('<td align="center" valign="bottom">.*?</td>', '',html_str, flags=re.S)
    html_str = re.sub('<td width="21" align="left" valign="top".*?</td>','',html_str,flags=re.S)
    html = html_str.encode(chard,'ignore')
    # print(html_str)
    return html,chard,html_str

#获取正文标题、附件信息，并下载附件，参数为分页面url，网页编码格式        #不使用
...
def get_file(html_str,href,file_ad):
    ...
    for each in file_infos:
        ...
        if re.findall('http',file_href):
            ...
            file_href = 'http://www.zhkgmx.gov.cn' + file_href
        else:
            href_add = href.replace(href.split('/')[-1], '')
            file_href =href_add + file_href[2:]
        print(file_href,file_name)
    ...
    for each in img_infos:
        ...
        if re.findall('http',img_href):
            ...
            img_href = 'http://www.zhkgmx.gov.cn/' + img_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
    ...
    for each in css_infos:
        ...
        if re.findall('http',css_href):
            ...
            css_href = 'http://www.zhkgmx.gov.cn/' + css_href.replace('../','')
        else:
            href_add = href.replace(href.split('/')[-1], '')
...
#珠海市科技和工业信息化局 通知公告+政策文件+政策解读    静态网页
def tztg_url(source1,source2,source3,row,worksheet,url,href_bloom,file_ad,ProgramStarttime):
    ...
    soup = beautiful(req, 'html.parser')
    ul = soup.find('td', {'width': '676'})
    #日期，链接，标题，标题
    href_list = ul.find_all('table',{'width': '651'})
    for h in href_list:
        date = h.find('td',{'width':'80'}).text[1:-1]
        date = date.replace('.', '-').replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
        date = datetime.datetime.strptime(date, '%Y-%m-%d')
        href_re = h.find('td',{'width':'554'}).a['href']
        if '../../' in href_re:
            complete_href = 'http://www.zhkgmx.gov.cn/' + href_re.replace('../','')
        elif '../' in href_re:
            complete_href = url.replace(url.split('/')[-1],'')[:-1].replace(url.split('/')[-2],'')+href_re.replace('../','')
        elif  './' in href_re:
            complete_href = url.replace(url.split('/')[-1],'')+href_re.replace('./','')
        else:
            complete_href = href_re
        down_href = 'http://www.zhkgmx.gov.cn'
        title = h.find('td',{'width':'554'}).a.text
...
def main(row, worksheet, href_bloom, file_ad1, ProgramStarttime):
    href_list = []
    url_list = ['http://www.zhkgmx.gov.cn/zwgk/tzgg/','http://www.zhkgmx.gov.cn/zwgk/wjk/',
                'http://www.zhkgmx.gov.cn/zwgk/zcjd/']
    source1 = '科工信局'
    source2_list = ['通知公告','政策文件','政策解读']
    source3_list = ['通知公告','政策法规','政策解读']
